...
public class CityTxAdapter extends RecyclerView.Adapter<CityTxAdapter.ViewHolder> {//该适配器继承字RecyclerView的Adapter适配器，因其具有可以指定
    private List<CityTx> mTxList;//用以将适配完的子项储存的链表，它的泛型是之前的实体类
    ...
    static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View view) {
            ...
            txImage = view.findViewById(R.id.im_tx);
            txName = view.findViewById(R.id.tx_name);//通过R文件的id查找，找出子项的具体控件
        }
    }

    public CityTxAdapter(List<CityTx> txList) {
        //链表的赋值
        mTxList = txList;
    }
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.interested_city, parent, false);
        //将子项的布局通过LayoutInflater引入
        final CityTxAdapter.ViewHolder holder = new CityTxAdapter.ViewHolder(view);
    ...
    @Override
    public void onBindViewHolder(CityTxAdapter.ViewHolder holder, int position) {
        //用以将滚入屏幕的子项加载图片等的方法，两个参数
        // A:前面方法ViewHolder的对象；
        //B:子项的id
        CityTx tx = mTxList.get(position);//创建前面实体类的对象
